...
searchBtn.addEventListener("click", searchOrganizer);

function searchOrganizer() {
    console.log("searchOrganizer");
    let noviOrganizatori = {};
    let noviIds = [];
    let nameToSearch = document.getElementById("naziv-organizatora-search").value;
    for (let id of organizatoriFestivalaIds) {
        if (
            organizatoriFestivala[id].naziv
            .toLowerCase()
            .includes(nameToSearch.trim().toLowerCase())
        ) {
            noviOrganizatori[id] = organizatoriFestivala[id];
        }
    }

    for (let id in noviOrganizatori) {
        noviIds.push(id);
    }

    console.log("stari organizatori", organizatoriFestivala);
    organizatoriFestivala = noviOrganizatori;
    organizatoriFestivalaIds = noviIds;
    console.log("novi organizatori", organizatoriFestivala);
    showOrganizatori();
}